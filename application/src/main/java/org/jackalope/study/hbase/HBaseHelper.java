...
public class HBaseHelper {
    ...
    public final static String defaultCoClassName = "com.iuni.data.analyze.hbase.sindex.IuniIndexRegionObserve";
    public final static String defaultCoJarFilePath = "hdfs:///hbase/coprocessor/analyze-1.0-SNAPSHOT.jar";
    public final static int defaultCoPriority = 1001;
    ...
    public void createTableWithCoprocessor(String table, byte[][] splitKeys, String... colfams)
            throws IOException {
        createTableWithCoprocessor(table, splitKeys, defaultCoClassName, defaultCoJarFilePath, defaultCoPriority, null, colfams);
    }

    public void createTableWithCoprocessor(String table, byte[][] splitKeys, String className, String jarFilePath, int priority, java.util.Map<java.lang.String,java.lang.String> kvs, String... colfams)
            throws IOException {
        HTableDescriptor desc = new HTableDescriptor(table);
        for (String cf : colfams) {
            HColumnDescriptor coldef = new HColumnDescriptor(cf);
        ...
        if (className != null && jarFilePath != null){
            desc.addCoprocessor(className, new Path(jarFilePath), priority, kvs);
        }
        if (splitKeys != null) {
            admin.createTable(desc, splitKeys);
        } else {
